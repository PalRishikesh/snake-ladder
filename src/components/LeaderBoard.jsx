...
const LeaderBoard = () => {
  ...
  const onDiceRollEvent = () => {
    ...
    setUserRandomNumber(userRandomNumber + diceRandomNumber);
    // rollCoinBaseOnDice();
  };

  const rollCoinBaseOnDice = () => {
    // console.log("Roll dice", randomNumber);
    // console.log("Roll User ", userRandomNumber);
  };
  // useE

  const allCalculation=()=>{
    // console.log("use Effect random number : ", randomNumber);
    ...
    if(ladders[updatedUserRandomNumber]){
     ...
     if(snakes[updatedUserRandomNumber]){
      ...
      console.log("updated value with snakes: ",ladders[updatedUserRandomNumber]);
    }
  }
  useEffect(() => {
    allCalculation();
      ...
      {rows.map((row, index) => (
        <div key={index} className="tile-row">
          {row.map((singleColumns) => {
            return (
              <>
                <div key={singleColumns} className={"tile x"+singleColumns} data-id={singleColumns}>
